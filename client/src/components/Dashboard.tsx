...
const dashboardItems = [
	...
	{ label: 'MARKET', icon: '📈', route: '/market' }, // Added route to Market
				...
				<div className='dashboard-action-row'>
					{mainButtons.slice(0, 4).map((item) => (
						<button
							...
							key={item.label}
							tabIndex={0}
							disabled={fundsLocked && (item.label === 'DEPOSIT' || item.label === 'WITHDRAW' || item.label === 'TRANSFER')}
							onClick={
			...
			<div className='dashboard-circles-grid'>
				{dashboardItems.map((item) => (
					<button
						...
						tabIndex={0}
						onClick={() => {
							if (item.route) window.location.href = item.route; // Ensure SETTINGS navigates to its route
						}}
					>
						<span className='dashboard-circle-icon'>{item.icon}</span>
						<span className='dashboard-circle-label'>{item.label}</span>
