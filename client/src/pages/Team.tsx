  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fff' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', marginTop: 30, gap: 20 }}>
        ...
        >
          <h2 style={{ fontSize: '1.1rem', marginBottom: 4, fontWeight: 700, color: '#25324B', letterSpacing: 1 }}>Your Referral Link</h2>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', justifyContent: 'center' }}>
            <input
              type="text"
              value={copied ? 'Copied!' : referralLink}
              readOnly
              style={{ width: '90%', padding: 8, border: '1px solid #ccc', fontSize: 15, background: '#f7faff', borderRadius: 0, cursor: 'pointer', color: copied ? '#1e3c72' : undefined, fontWeight: copied ? 600 : undefined }}
        ...
        >
          <div style={{ fontWeight: 700, color: '#25324B', fontSize: '1.1rem', letterSpacing: 1 }}>Total Members: {members.length}</div>
        </div>
        {/* Members List Cards */}
        {loading ? (
          ...
          members.map((member: any) => (
            ...
            >
              <div style={{ fontWeight: 700, color: '#25324B', fontSize: '1.05rem', marginBottom: 4 }}>{member.fullName}</div>
              <div style={{ color: '#555', fontSize: '0.97rem', marginBottom: 2 }}>{member.email}</div>
              <div style={{ color: '#888', fontSize: '0.93rem', marginBottom: 2 }}>Joined: {new Date(member.joinedAt).toLocaleDateString()}</div>
              <div style={{ fontSize: 18, marginTop: 2 }}>Active Stock: {member.activeStock ? '✅' : '❌'}</div>
            </div>
          ))
        )}
        <style>
