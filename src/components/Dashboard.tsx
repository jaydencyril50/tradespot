...
const ConvertModal: React.FC<ConvertModalProps> = ({
  ...
  // New: local error for insufficient balance
  ...
  React.useEffect(() => {
    ...
    if (convertDirection === 'USDT_TO_SPOT') {
      if (Number(convertAmount) > usdtBalance) {
        setInputError('Insufficient balance');
        return;
      }
    } else {
      if (Number(convertAmount) > spotBalance) {
        setInputError('Insufficient balance');
        return;
      }
    }
    setInputError('');
  }, [convertAmount, convertDirection, usdtBalance, spotBalance]);
