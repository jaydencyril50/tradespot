...
const BuySpotPage: React.FC = () => {
  ...
  // Fetch logged-in user's USDT balance and VIP level
  ...
  // Poll user USDT balance every 5 seconds
  useEffect(() => {
    const interval = setInterval(() => {
      fetchUserBalance();
    }, 5000); // 5 seconds
  ...
  // Only show buyers matching user's VIP level, sorted by minLimit ascending
  ...
  // Poll trade limits every 12 hours and update min/max limits for each buyer
  useEffect(() => {
    if (!buyers.length) return;
    const updateTradeLimits = async () => {
      try {
        // Fetch latest buyers from backend
        const res = await axios.get(`${API}/api/p2p/buyers`);
        const latestBuyers = res.data;
        setBuyers(prevBuyers => prevBuyers.map(buyer => {
          const updated = latestBuyers.find((b: any) => b._id === buyer._id);
          if (updated) {
            return {
              ...buyer,
  ...
  const handleBuySpot = async () => {
    ...
    try {
      ...
      if (msg.includes('You can only complete 1 buy order per day')) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fff' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', marginTop: 12, gap: 10 }}>
        ...
        {/* Warning for daily buy order limit */}
        {dailyLimitWarning && (
          <div style={{
            color: '#d32f2f',
            background: '#fff3f3',
            ...
            return (
              ...
              >
                ...
                <div style={{ fontSize: '1.05rem', fontWeight: 600, marginBottom: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0 }}>
                  ...
                  <strong style={{ color: buyer.status === 'online' ? '#27ae60' : buyer.status === 'recently' ? '#f1c40f' : '#888', marginRight: 8 }}>
                    {buyer.status.charAt(0).toUpperCase() + buyer.status.slice(1)}
                  </strong>
                  <span style={{ fontWeight: 600, color: '#25324B', fontSize: '1.05rem', display: 'flex', alignItems: 'center', gap: 4 }}>
